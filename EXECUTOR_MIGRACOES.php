<?php
// EXECUTOR_MIGRACOES.php
// Executor de migrações SQL no banco de dados

echo "<h1>🚀 EXECUTOR DE MIGRAÇÕES</h1>";
...
$migracoes = glob($migrations_dir . '/*.sql');
sort($migracoes);

echo "<h2>2. 📝 Executando migrações...</h2>";
echo "<p>Encontradas " . count($migracoes) . " migrações</p>";
...
foreach ($migracoes as $migracao) {
    $nome_arquivo = basename($migracao);
    echo "<h3>📄 Executando: $nome_arquivo</h3>";
...
file_put_contents('/tmp/execucao_migracoes.json', json_encode($log, JSON_PRETTY_PRINT));

echo "<h2>💾 Log de execução salvo em /tmp/execucao_migracoes.json</h2>";
?>
