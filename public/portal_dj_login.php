...
/**
 * portal_dj_login.php
 * Login do Portal DJ (acesso externo)
 */
...
if (!empty($_SESSION['portal_dj_logado']) && $_SESSION['portal_dj_logado'] === true) {
    header('Location: index.php?page=portal_dj');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$login || !$senha) {
        ...
        $stmt = $pdo->prepare("
            ...
            WHERE login = :login AND tipo = 'dj' AND ativo = TRUE
        ");
        ...
        if (!$fornecedor) {
            ...
            $_SESSION['portal_dj_logado'] = true;
            $_SESSION['portal_dj_fornecedor_id'] = $fornecedor['id'];
            $_SESSION['portal_dj_nome'] = $fornecedor['nome'];
            $_SESSION['portal_dj_token'] = $token;
