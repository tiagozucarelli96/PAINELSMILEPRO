    ...
    if ($login === '' || $senha === '') {
        ...
        try {
            ...
            if (!$u) {
                ...
                $hash = (string)$u['senha'];
                $ok = false;
                if ($hash === $senha) $ok = true;                                // texto puro (legado)
                elseif (strlen($hash) === 32 && $hash === md5($senha)) $ok = true; // md5 (legado)
                elseif (password_get_info($hash)['algoName'] ?? false) $ok = password_verify($senha, $hash); // hash moderno
                ...
                if (!$ok) {
                    ...
                    header('Location: /index_v2.php?page=dashboard');
                    exit;
                }
            }
        } catch (Throwable $e) {
...
<body>
  <form class="card" method="post" action="/login_v2.php" autocomplete="off">
    <h1>Entrar</h1>
