<?php
// gerar_lista_compras_v2.php - Sistema de rascunho para múltiplos eventos
session_start();
require_once __DIR__ . '/conexao.php';

function h($s) { return htmlspecialchars((string)$s, ENT_QUOTES, 'UTF-8'); }

// Inicializar rascunho na sessão se não existir
if (!isset($_SESSION['rascunho_lista'])) {
    $_SESSION['rascunho_lista'] = [
        'eventos' => [],
        'itens_consolidados' => []
    ];
}
    ...
    try {
        switch ($action) {
            case 'adicionar_evento':
                ...
                foreach ($_SESSION['rascunho_lista']['eventos'] as $evento) {
                    if ($evento['id'] == $evento_id) {
                        throw new Exception('Este evento já foi adicionado ao rascunho.');
                    }
                }
                
                // Buscar dados do evento
                $stmt = $pdo->prepare("
                    SELECT * FROM smilee12_painel_smile.lc_listas_eventos 
                    WHERE id = ?
                ");
                ...
                $itens_evento = [];
                foreach ($itens_selecionados as $key => $item_id) {
                    $quantidade = (float)($_POST['quantidades'][$key] ?? 1);
                    if ($quantidade > 0) {
                        $tipo = strpos($key, 'insumo_') === 0 ? 'insumo' : 'receita';
                        $itens_evento[] = [
                            'tipo' => $tipo,
                            'id' => $item_id,
                ...
                $_SESSION['rascunho_lista']['eventos'][] = [
                    'id' => $evento['id'],
                    'nome' => $evento['evento'],
                    'data' => $evento['data'],
                ...
                $msg = "Evento '{$evento['evento']}' adicionado ao rascunho!";
                break;
                
            case 'remover_evento':
                $evento_id = (int)$_POST['evento_id'];
                
                // Remover evento do rascunho
                $_SESSION['rascunho_lista']['eventos'] = array_filter(
                    $_SESSION['rascunho_lista']['eventos'],
                    function($evento) use ($evento_id) {
                ...
                $msg = 'Evento removido do rascunho.';
                break;
            ...
            case 'gerar_lista_final':
                if (empty($_SESSION['rascunho_lista']['eventos'])) {
                    throw new Exception('Adicione pelo menos um evento ao rascunho.');
                }
                
                // Aqui você pode salvar a lista final no banco
...
function consolidarItens() {
    ...
    foreach ($_SESSION['rascunho_lista']['eventos'] as $evento) {
        foreach ($evento['itens'] as $item) {
            ...
            if (!isset($itens_consolidados[$key])) {
            ...
            $itens_consolidados[$key]['quantidade_total'] += $item['quantidade'];
            $itens_consolidados[$key]['eventos'][] = [
                'evento_id' => $evento['id'],
    ...
    $_SESSION['rascunho_lista']['itens_consolidados'] = $itens_consolidados;
}

// Carregar dados
try {
    // Carregar eventos disponíveis (excluindo os já adicionados)
    $eventos_adicionados = array_column($_SESSION['rascunho_lista']['eventos'], 'id');
    $eventos_where = empty($eventos_adicionados) ? '' : 'WHERE id NOT IN (' . implode(',', $eventos_adicionados) . ')';
    ...
    <div class="main-content">
        <div class="container">
            <div class="page-header">
                ...
                <p class="page-subtitle">Adicione eventos ao rascunho e gere sua lista consolidada</p>
            </div>
            ...
            <!-- Rascunho da Lista -->
            <div class="rascunho-section">
                <div class="rascunho-header">
                    <h2 class="rascunho-title">📝 Rascunho da Lista</h2>
                    <div class="rascunho-actions">
                        <?php if (!empty($_SESSION['rascunho_lista']['eventos'])): ?>
                            <form method="post" style="display: inline;">
                                <input type="hidden" name="action" value="limpar_rascunho">
                                <button type="submit" class="btn btn-outline btn-sm" 
                                        onclick="return confirm('Tem certeza que deseja limpar o rascunho?')">
                                    🗑️ Limpar
                                </button>
                            </form>
                        <?php endif; ?>
                ...
                <?php if (empty($_SESSION['rascunho_lista']['eventos'])): ?>
                    <div class="empty-state">
                        ...
                        <p>Nenhum evento adicionado ao rascunho.</p>
                        <p class="text-sm">Selecione um evento e adicione itens para começar.</p>
                    </div>
                <?php else: ?>
                    <?php foreach ($_SESSION['rascunho_lista']['eventos'] as $evento): ?>
                        <div class="evento-card">
                            <div class="evento-info">
                                <h4 class="evento-nome"><?= h($evento['nome']) ?></h4>
                                <p class="evento-detalhes">
                                    📅 <?= date('d/m/Y', strtotime($evento['data'])) ?> | 
                                    👥 <?= $evento['convidados'] ?> convidados | 
                                    📦 <?= count($evento['itens']) ?> itens
                                </p>
                            </div>
                            <div class="evento-actions">
                                <form method="post" style="display: inline;">
                                    <input type="hidden" name="action" value="remover_evento">
                    ...
                    <!-- Resumo Consolidado -->
                    <?php if (!empty($_SESSION['rascunho_lista']['itens_consolidados'])): ?>
                        <div class="summary-card">
                            <h3 class="summary-title">📊 Resumo Consolidado</h3>
                            <div class="summary-stats">
                                <div class="summary-stat">
                                    <div class="summary-stat-value"><?= count($_SESSION['rascunho_lista']['eventos']) ?></div>
                                    <div class="summary-stat-label">Eventos</div>
                                </div>
                                <div class="summary-stat">
                                    <div class="summary-stat-value"><?= count($_SESSION['rascunho_lista']['itens_consolidados']) ?></div>
                                    <div class="summary-stat-label">Itens Únicos</div>
                                </div>
                                <div class="summary-stat">
                                    <div class="summary-stat-value">
                                        <?= array_sum(array_column($_SESSION['rascunho_lista']['itens_consolidados'], 'quantidade_total')) ?>
                                    </div>
                                    <div class="summary-stat-label">Total de Quantidades</div>
                                </div>
                            </div>
                        </div>
                    <?php endif; ?>
            ...
            <?php if (!empty($eventos)): ?>
                <form method="post" id="adicionarEventoForm">
                                            ...
                                            <div class="item-card" onclick="toggleItem(this)">
                                                ...
                                                <div class="item-details">
                                                    ...
                                                    <div class="item-detail">
                                                        <span class="item-detail-icon">🔢</span>
                                                        <input type="number" 
                                                               name="quantidades[insumo_<?= $item['id'] ?>]"
                                            ...
                                            <div class="item-card" onclick="toggleItem(this)">
                                                ...
                                                <div class="item-details">
                                                    ...
                                                    <div class="item-detail">
                                                        <span class="item-detail-icon">🔢</span>
                                                        <input type="number" 
                                                               name="quantidades[receita_<?= $item['id'] ?>]"
                                                               class="quantity-input" 
                                                               placeholder="Qtd" 
                                                               min="0" 
                                                               step="0.001"
                    ...
                    <div class="flex gap-4 justify-center mt-8">
                        ...
                        <button type="submit" class="btn btn-primary" id="adicionarBtn" disabled>
                            <span>➕</span> Adicionar Evento ao Rascunho
                        </button>
                    </div>
                </form>
            <?php else: ?>
                <div class="text-center py-8 text-gray-500">
                    <p>Nenhum evento disponível para adicionar.</p>
                </div>
            <?php endif; ?>
            ...
            <?php if (!empty($_SESSION['rascunho_lista']['eventos'])): ?>
                <div class="text-center mt-8">
                    <form method="post" style="display: inline;">
                        <input type="hidden" name="action" value="gerar_lista_final">
                        <button type="submit" class="btn btn-success btn-lg">
                            <span>📋</span> Gerar Lista de Compras Final
                        </button>
                    </form>
                </div>
            <?php endif; ?>
