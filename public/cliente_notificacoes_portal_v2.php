<?php
/**
 * Tela dedicada para a campanha "Lançamento do Portal do Cliente".
 */
...
if (empty($erro) && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') !== 'enviar_campanha_portal') {
    try {
        ...
        if (!$modelo) {
            throw new RuntimeException('Modelo de lançamento não encontrado.');
        }
...
.notif-btn { border: 0; border-radius: 8px; padding: .75rem 1rem; font-weight: 800; cursor: pointer; background: #0f766e; color: #fff; text-decoration: none; display: inline-flex; align-items: center; }
.notif-btn.secondary { background: #e2e8f0; color: #0f172a; }
...
.notif-audience, .notif-code-list { padding: .9rem 1.1rem 1.1rem; overflow-x: auto; }
...
.notif-meta { display: grid; grid-template-columns: 1fr; gap: 1rem; margin-top: 1rem; }
.notif-code-list { display: grid; gap: .55rem; }
...
@media (max-width: 960px) { .notif-layout, .notif-grid, .notif-campaign-summary { grid-template-columns: 1fr; } .notif-campaign-actions { align-items: stretch; flex-direction: column; } }
</style>
...
<div class="notif-page">
    ...
    <div class="notif-layout">
        <aside class="notif-panel">
            <div class="notif-panel-head"><h2>Modelos</h2><span>Novas regras futuras entram nesta lista.</span></div>
            <div class="notif-list">
                <?php foreach ($modelos as $modelo): ?>
                    <?php $href = ((string)$modelo['chave'] === 'portal_cliente_lancamento') ? 'cliente_notificacoes_portal_v2.php' : ('cliente_notificacoes.php?modelo_chave=' . urlencode((string)$modelo['chave'])); ?>
                    <a class="notif-list-item <?= (string)$modelo['chave'] === 'portal_cliente_lancamento' ? 'active' : '' ?>" href="<?= h($href) ?>">
        ...
        <div>
            <section class="notif-panel">
                <div class="notif-panel-head"><h2><?= h($modeloAtual['nome']) ?></h2><span><?= h($modeloAtual['descricao']) ?></span></div>
                <form method="post" class="notif-form">
                    <div class="notif-grid">
                        <div class="notif-field"><label for="nome">Nome da notificação</label><input type="text" id="nome" name="nome" value="<?= h($modeloAtual['nome']) ?>" required></div>
                        <div class="notif-field"><label for="gatilho">Quando dispara</label><input type="text" id="gatilho" name="gatilho" value="<?= h($modeloAtual['gatilho']) ?>"></div>
                    </div>
                    <div class="notif-field"><label for="descricao">Descrição interna</label><input type="text" id="descricao" name="descricao" value="<?= h($modeloAtual['descricao']) ?>"></div>
                    ...
                    <div class="notif-grid">
                        <div class="notif-field"><label for="assunto">Assunto do e-mail</label><input type="text" id="assunto" name="assunto" value="<?= h($modeloAtual['assunto']) ?>" required></div>
                        <div class="notif-field"><label for="botao_texto">Texto do botão</label><input type="text" id="botao_texto" name="botao_texto" value="<?= h($modeloAtual['botao_texto']) ?>" required></div>
                    </div>
                    <div class="notif-field"><label for="mensagem_texto">Mensagem</label><textarea id="mensagem_texto" name="mensagem_texto" required><?= h($modeloAtual['mensagem_texto']) ?></textarea></div>
                    <div class="notif-actions">
                        <a class="notif-btn secondary" href="cliente_notificacoes.php?modelo_chave=portal_cliente_lancamento&preview=1" target="_blank" rel="noopener">Ver prévia</a>
            ...
            <section class="notif-panel notif-campaign">
                <div class="notif-panel-head"><h2>Disparo em massa</h2><span>Consulta direta na ME Eventos: eventos futuros a partir de 30/05/2026, apenas casamento e 15 anos, sem envio anterior desta campanha.</span></div>
                ...
                <div class="notif-campaign-actions">
                    <div class="notif-warning">A tela abre sem consultar a ME Eventos para não travar. Primeiro clique em consultar público; depois confira a lista e envie. No envio, o sistema cria automaticamente o portal para quem ainda não tiver e dispara o e-mail com o link individual.</div>
                    <div style="display:flex;gap:.6rem;flex-wrap:wrap;justify-content:flex-end;">
                        <a class="notif-btn secondary" href="cliente_notificacoes_portal_v2.php?consultar_publico=1">Consultar público na ME</a>
                        ...
                        <tbody>
                            ...
                            <?php foreach (array_slice($publicoCampanha, 0, 80) as $item): ?>
                                <tr>
                                    ...
                                    <td><span class="notif-badge <?= !empty($item['portal_url']) ? 'green' : '' ?>"><?= !empty($item['portal_url']) ? 'Criado' : 'Será criado' ?></span></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
            ...
            <section class="notif-panel notif-meta">
                <div class="notif-panel-head"><h2>Legenda dos códigos</h2><span>Use estes códigos no assunto ou na mensagem.</span></div>
                <div class="notif-code-list">
                    <?php foreach ($codigos as $codigo => $descricao): ?>
                        <div class="notif-code"><code><?= h($codigo) ?></code><span><?= h($descricao) ?></span></div>
                    <?php endforeach; ?>
                </div>
            </section>
        </div>
