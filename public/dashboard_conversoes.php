<?php
// dashboard_conversoes.php — Dashboard com sistema de conversões
...
try {
    ...
    // Buscar dados de conversões das degustações
    try {
        // Total de inscrições
        $total_inscricoes = $pdo->query("SELECT COUNT(*) FROM comercial_inscricoes")->fetchColumn();
        
        // Contratos fechados (fechou_contrato = 'sim')
        $contratos_fechados = $pdo->query("SELECT COUNT(*) FROM comercial_inscricoes WHERE fechou_contrato = 'sim'")->fetchColumn();
        
        // Pagamentos realizados
        $pagamentos_realizados = $pdo->query("SELECT COUNT(*) FROM comercial_inscricoes WHERE pagamento_status = 'pago'")->fetchColumn();
        
        // Calcular taxa de conversão
        $taxa_conversao = $total_inscricoes > 0 ? round(($contratos_fechados / $total_inscricoes) * 100, 1) : 0;
        
        $stats['leads_total'] = $total_inscricoes;
        $stats['contratos_fechados'] = $contratos_fechados;
        $stats['vendas_realizadas'] = $pagamentos_realizados;
        $stats['conversoes'] = $contratos_fechados;
        $stats['taxa_conversao'] = $taxa_conversao;
        
        // Buscar eventos ativos
        $stats['eventos'] = $pdo->query("SELECT COUNT(*) FROM comercial_degustacoes WHERE status = 'publicado'")->fetchColumn();
        
    } catch (Exception $e) {
        $erros[] = "Erro ao buscar dados de conversões: " . $e->getMessage();
    }
    
    // Buscar dados da ME Eventos (webhooks)
    try {
        $mes_atual = date('Y-m');
        ...
        if ($me_stats) {
            // Combinar dados da ME Eventos com dados internos
...
<div class="dashboard-container">
    ...
    <!-- Resumo Comercial com Conversões -->
    <div class="dashboard-card">
        <h3 style="color: #1e3a8a; margin-bottom: 20px; font-size: 1.5em;">📊 Resumo Comercial & Conversões</h3>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px;">
            <div style="text-align: center;">
                <div style="font-size: 2em; color: #1e3a8a; font-weight: bold;"><?= $stats['leads_total'] ?></div>
                <div style="color: #64748b;">Total Inscrições</div>
            </div>
            <div style="text-align: center;">
        ...
        <div style="margin-top: 30px; padding: 20px; background: #f8fafc; border-radius: 10px; text-align: center;">
            <h4 style="color: #1e3a8a; margin-bottom: 15px;">🎯 Taxa de Conversão</h4>
            <div style="font-size: 3em; color: #1e3a8a; font-weight: bold; margin-bottom: 10px;">
                <?= $stats['taxa_conversao'] ?>%
            </div>
            <div style="color: #64748b;">
                <?= $stats['contratos_fechados'] ?> contratos fechados de <?= $stats['leads_total'] ?> inscrições
            </div>
        </div>
    </div>
