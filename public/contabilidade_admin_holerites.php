...
if ($filtro_mes) {
    $where_conditions[] = "mes_competencia = :mes";
    $params[':mes'] = $filtro_mes;
}

if ($filtro_ajuste === 'sim') {
    $where_conditions[] = "e_ajuste = TRUE";
} elseif ($filtro_ajuste === 'nao') {
    $where_conditions[] = "e_ajuste = FALSE";
}
...
$holerites = [];
try {
    $sql = "SELECT * FROM contabilidade_holerites $where_sql ORDER BY mes_competencia DESC, criado_em DESC";
            ...
            <tr>
                ...
                <td>
                    <?php if ($holerite['arquivo_url']): ?>
                        <a href="<?= htmlspecialchars($holerite['arquivo_url']) ?>" target="_blank" class="btn-action btn-download">📎 Baixar</a>
                    <?php else: ?>
                        -
                    <?php endif; ?>
                </td>
